...
export async function getQueueStatus(): Promise<{ registrationQueueLength: number; processingQueueLength: number }> {
  await registrationQueueClient.connect();
  const client = registrationQueueClient.getClient();
  const registrationQueueLength = await client.lLen(REGISTRATION_QUEUE);
  const processingQueueLength = await client.lLen(PROCESSING_QUEUE);
  return { registrationQueueLength, processingQueueLength };
}
