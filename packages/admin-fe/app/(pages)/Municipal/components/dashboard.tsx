...
// ─── Main Component ───────────────────────────────────────────────────
// ═══════════════════════════════════════════════════════════════════════
export function MunicipalDashboard({ onTabChange }: { onTabChange?: (tab: 'dashboard' | 'my-complaints' | 'reports' | 'agent-management' | 'announcements') => void }) {
  // ── State ──
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/municipal-admin/complaints/${selectedComplaint.id}/status`, {
        method: 'PUT',
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setSelectedComplaint((prev) =>
          prev ? { ...prev, status: updated.status, escalationLevel: 'STATE_ADMIN', managedByMunicipalAdmin: null } : prev
        )
  ...
  const escalatedCount = complaints.filter((c) =>
    c.status?.includes('ESCALATED') || !!c.managedByMunicipalAdmin?.id || !!c.escalationLevel
  ).length
  ...
  const displayedComplaints = complaints.filter((complaint) => {
    ...
    if (assignmentFilter === 'escalated') return (
      !!complaint.managedByMunicipalAdmin?.id || !!complaint.escalationLevel ||
  ...
  const canUpdateStatus = adminType === 'MUNICIPAL_ADMIN' || adminType === 'STATE_ADMIN' || adminType === 'SUPER_ADMIN'
  ...
  return (
    <div className="p-6 md:p-8 flex flex-col gap-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black text-[#041627] tracking-tight">Complaints Registry</h2>
          <p className="text-[#44474c] text-sm font-medium mt-1">Manage and resolve citizen grievances across departments.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <span className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.08) 0%, transparent 60%)' }} />
          ...
          <tbody className="divide-y divide-[#c4c6cd]/5">
            {loading ? (
              ...
              displayedComplaints.map((c) => {
                const assignedName = c.assignedAgent?.name || c.managedByMunicipalAdmin?.name
                return (
                  <tr key={c.id} className="hover:bg-[#f3f4f5] transition-colors group">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {!c.assignedAgent?.id && !c.managedByMunicipalAdmin?.id && (
                          <button
            ...
            <div className="px-8 py-4 bg-[#f3f4f5] flex flex-wrap items-center justify-between gap-3">
              ...
              {isAssignedToCurrentMunicipalAdmin ? (
                ...
                >
                  ...
                  {statusUpdating ? 'Escalating...' : 'Escalate to State'}
                </button>
              ) : (
                <button
                  disabled
                  className="px-6 py-2 bg-[#e7e8e9] text-[#74777d] text-sm font-semibold rounded-lg cursor-not-allowed flex items-center gap-2"
                  title="Only the assigned municipal admin can escalate"
                >
                  <span className="material-symbols-outlined text-sm">priority_high</span> Escalate to State
                </button>
              )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8">
              <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-12 lg:col-span-8 space-y-6">
                  ...
                  {/* Assignment Info */}
                  {(selectedComplaint.assignedAgent || selectedComplaint.managedByMunicipalAdmin) && (
                    <section className="bg-white p-6 rounded-xl">
                      <h3 className="text-sm font-black uppercase tracking-widest text-[#44474c] flex items-center gap-2 mb-4">
                ...
                <aside className="col-span-12 lg:col-span-4 space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <MetaBlock label="Status"><StatusPill status={selectedComplaint.status} /></MetaBlock>
                    <MetaBlock label="Priority"><PriorityPill urgency={selectedComplaint.urgency} /></MetaBlock>
                  </div>
                  ...
                  {selectedComplaint.attachmentUrl && (
                    <section className="bg-white p-6 rounded-xl">
                      <h3 className="text-sm font-black uppercase tracking-widest text-[#44474c] mb-4 flex items-center gap-2">
                        <span className="material-symbols-outlined text-[#74777d]">attach_file</span> Attachments
                      </h3>
                      <div className="space-y-3">
                        <div className="flex items-center gap-3 p-2 bg-[#f8f9fa] hover:bg-[#edeeef] transition-colors rounded-lg group">
                          <div className="w-10 h-10 rounded bg-[#ffdad6] flex items-center justify-center text-[#ba1a1a]">
                            <span className="material-symbols-outlined">image</span>
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-[#041627] truncate">complaint_evidence.jpg</p>
                            <p className="text-[10px] text-[#44474c]">Uploaded by complainant</p>
                          </div>
                          <a href={selectedComplaint.attachmentUrl} target="_blank" rel="noopener noreferrer" className="material-symbols-outlined text-[#74777d] opacity-0 group-hover:opacity-100 transition-opacity">download</a>
                        </div>
                        <img src={selectedComplaint.attachmentUrl} alt="Complaint attachment" className="max-w-full h-auto rounded-lg" />
                      </div>
                    </section>
                  )}
                </aside>
...
// Small helper for meta blocks in the modal
function MetaBlock({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="bg-white p-4 rounded-xl">
      <p className="text-[10px] font-bold uppercase tracking-[0.05em] text-[#44474c] mb-1.5">{label}</p>
      {children}
    </div>
  )
}
