...
export async function runFeatureProbes(): Promise<CheckResult[]> {
  ...
  const probeDefinitions: Array<() => Promise<CheckResult>> = [
    ...
    probe({ id: 'feat-abe-autoassign-bc-pop', name: 'admin-be: POST Blockchain Queue Pop', group: 'feature-api', url: `${adminBe}/api/auto-assign/blockchain-queue-pop?probe=true`, method: 'POST', headers: agentHeaders, validate: aliveValidator, timeout: T_BLOCKCHAIN }),
