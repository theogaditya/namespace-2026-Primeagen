...
import {
  ...
  HelpCircle, LogOut, Menu, Bell, Search, ChevronRight
} from "lucide-react"
...
type TabKey = 'dashboard' | 'my-complaints' | 'reports' | 'agent-management' | 'announcements'
...
const NAV_SECTIONS: { title: string; items: { id: string; label: string; icon: React.ComponentType<{className?:string}>; tabKey: TabKey }[] }[] = [
  ...
  { title: "Administration", items: [
    { id: "agent-management", label: "Agent Management", icon: Users, tabKey: "agent-management" },
    { id: "announcements", label: "Announcements", icon: Megaphone, tabKey: "announcements" },
  ]},
]
...
export function RevampedLayout({ children, activeTab = 'dashboard', onTabChange }: RevampedLayoutProps) {
  ...
  const [adminData, setAdminData] = useState<{
    ...
    adminType?: string; municipality?: string;
  } | null>(null)
  ...
  useEffect(() => {
    setMounted(true)
    const saved = localStorage.getItem("municipalSidebarCollapsed")
  ...
  const toggleCollapse = useCallback(() => {
    setIsCollapsed(prev => {
      const next = !prev
      localStorage.setItem("municipalSidebarCollapsed", String(next))
      return next
    })
  }, [])
        ...
        <div className={cn(
          ...
          {!isCollapsed && (
            <div className="flex flex-col">
              ...
              <span className="text-[9px] font-semibold text-slate-400 mt-0.5 tracking-wide">Municipal Admin Portal</span>
            </div>
          )}
        </div>
            ...
            {!isCollapsed ? (
              <div className="flex items-center px-3 py-2 rounded-xl bg-white hover:bg-slate-50 transition-colors border border-slate-100/60">
                ...
                <div className="flex-1 min-w-0 ml-2.5">
                  ...
                  <p className="text-[9px] font-semibold text-emerald-500 uppercase mt-1 tracking-widest leading-none">{adminData?.municipality || "Municipal Admin"}</p>
                </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right hidden sm:block">
                ...
                <p className="text-[9px] font-bold text-emerald-500 uppercase mt-1 tracking-widest leading-none">{adminData?.municipality || "Municipality"}</p>
              </div>
