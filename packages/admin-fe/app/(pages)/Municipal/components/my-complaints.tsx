...
function getSlaInfo(submissionDate: string, urgency: string, status: string) {
  const done = status === "COMPLETED" || status === "ESCALATED_TO_STATE_LEVEL"
  if (done) {
    return {
      label: "CLOSED",
      time: "—",
...
// ─── Main Component ────────────────────────────────────────────────────────────
// ═══════════════════════════════════════════════════════════════════════════════
export function MunicipalMyComplaints() {
  ...
  const fetchMyComplaints = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/municipal-admin/my-complaints?page=${pagination.currentPage}&limit=${pagination.itemsPerPage}`,
  ...
  const updateStatus = async (complaintId: string, status: string) => {
    ...
    try {
      const token = localStorage.getItem("token")
      if (!token) return
      const isEscalation = status === "ESCALATED_TO_STATE_LEVEL"
      const endpoint = isEscalation
        ? `${API_URL}/api/municipal-admin/complaints/${complaintId}/escalate`
        : `${API_URL}/api/municipal-admin/complaints/${complaintId}/status`
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-[#44474c] text-sm font-medium mt-1">
            Manage and resolve citizen grievances assigned to you.
          </p>
        </div>
        ...
        >
          ...
          <div className="px-8 py-4 bg-[#f3f4f5] flex items-center justify-between">
            ...
            {selectedComplaint.status !== "ESCALATED_TO_STATE_LEVEL" && (
              <button
                onClick={() =>
                  updateStatus(selectedComplaint.id, "ESCALATED_TO_STATE_LEVEL")
                }
              ...
              <section className="bg-white p-6 rounded-xl">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-[#f3f4f5] rounded-lg">
                    ...
                    <p className="text-sm font-semibold text-[#041627]">
                      {selectedComplaint.assignedAgent?.name || (
                        <span className="italic text-[#44474c]/40">Unassigned</span>
                      )}
                    </p>
                  </div>
