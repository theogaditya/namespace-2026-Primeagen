...
export function SuperAvailableComplaints() {
  ...
  const fetchAvailableComplaints = async (showLoader = true) => {
    ...
    try {
      ...
      // Super admin uses super-admin endpoint
      const response = await fetch(`${API_URL}/api/super-admin/complaints?${params}`, {
        headers: {
      ...
      if (data.success) {
        const filtered = (data.complaints as any[]).filter((c) => !c.isDuplicate)
        const duplicatesOnPage = (data.complaints as any[]).length - filtered.length
  ...
  const formatDate = (dateValue: string | number) => {
    const normalized = typeof dateValue === 'number' && dateValue < 1e12
      ? dateValue * 1000
      : dateValue
  ...
  const getHeaderDescription = () => {
    switch (assignmentFilter) {
      ...
      case 'all':
      default:
        return 'Overview of all complaints across the platform'
    }
  }
  ...
  const handleDeleteComplaint = async () => {
    ...
    if (!confirm('Delete this complaint? This will mark it as DELETED and cannot be undone.')) return
    setDeleting(true)
    try {
      const token = localStorage.getItem('token')
      if (!token) throw new Error('Not authenticated')

      const endpoint = `${API_URL}/api/super-admin/delete/${selectedComplaint.id}`
      ...
      if (!res.ok) {
        alert(body.message || body.error || 'Failed to delete complaint')
      } else {
        // Remove from local list and close modal
        setComplaints((prev) => prev.filter((c) => c.id !== selectedComplaint.id))
        setSelectedComplaint(null)
        setIsModalOpen(false)
        alert(body.message || 'Complaint deleted')
      }
    } catch (err: any) {
      console.error('Delete error', err)
      alert(err?.message || 'Unable to delete the complaint')
    } finally {
      setDeleting(false)
    }
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <p className="text-gray-600">Manage and track all complaints across the platform</p>
      </div>
                  ...
                  displayedComplaints.map((complaint) => (
                    <TableRow key={complaint.id}>
                      <TableCell>
                        <div>
                          ...
                          <div className="text-sm text-gray-500">
                            #{complaint.seq} • {typeof complaint.category === 'string' ? complaint.category : (complaint as any).category?.name || 'Uncategorized'}
                          </div>
                      ...
                      <TableCell className="text-center align-middle">
                        ...
                        >
                          <Eye className="h-4 w-4 text-red-500 group-hover:text-black transition-colors" />
                          <span className="text-red-500 group-hover:text-black transition-colors">View</span>
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))
          ...
          <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
            <div className="sticky top-0 z-10 bg-white pb-4 border-b shadow-sm -mx-6 px-6 -mt-6 pt-0">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">#{selectedComplaint?.seq} • {typeof selectedComplaint?.category === 'string' ? selectedComplaint?.category : (selectedComplaint as any)?.category?.name || 'Uncategorized'}</p>
            ...
            <div className="space-y-4 pt-9">
              ...
              {canUpdateStatus(selectedComplaint) ? (
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const token = localStorage.getItem('token')
                          if (!token) throw new Error('Not authenticated')
                          const endpoint = `${API_URL}/api/super-admin/complaints/${selectedComplaint.id}/status`
                ...
                <div className="border-t pt-4">
                  <p className="text-sm text-amber-600 bg-amber-50 p-3 rounded-md">Only Super Admins can Update Status.</p>
                </div>
              )}
            </div>
          </Modal>
...
function SuperComplaintLocationMap({ lat, lng, label }: { lat: number; lng: number; label?: string }) {
  useMemo(() => {
    try {
      // @ts-ignore
