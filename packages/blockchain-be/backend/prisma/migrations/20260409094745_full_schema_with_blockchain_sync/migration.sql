/*
  ...
  - Added the required column `isPublic` to the `Complaint` table without a default value. This is not possible if the table is not empty.
  ...
  - Added the required column `subCategory` to the `Complaint` table without a default value. This is not possible if the table is not empty.
...
-- DropIndex
DROP INDEX "Complaint_seq_idx";

-- AlterTable
ALTER TABLE "Complaint" ADD COLUMN     "AIabusedFlag" BOOLEAN,
...
ADD COLUMN     "isDuplicate" BOOLEAN,
...
ADD COLUMN     "sla" TEXT,
ADD COLUMN     "status" "ComplaintStatus" NOT NULL DEFAULT 'REGISTERED',
