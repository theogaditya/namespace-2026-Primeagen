...
export function AddAgentForm({ onSuccess, onCancel }: AddAgentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/municipal-admin/create/agent`, {
        method: "POST",
  ...
  if (success) {
    return (
      <div className="bg-white rounded-xl max-w-2xl mx-auto p-10 flex flex-col items-center justify-center text-center">
        ...
        <h3 className="text-xl font-black text-[#041627] tracking-tight mb-2">Agent Created Successfully!</h3>
        <p className="text-[#44474c] text-sm">The new agent has been added to the system.</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-xl max-w-2xl mx-auto overflow-hidden">
      ...
      <div className="px-6 py-5 bg-[#f3f4f5]/50 flex items-center gap-3">
        ...
        <div>
          <h3 className="text-lg font-bold text-[#041627] leading-tight">Create New Agent</h3>
          <p className="text-sm text-[#44474c] font-medium">Fill out the form to onboard a new municipal or department-level agent.</p>
        </div>
      </div>
